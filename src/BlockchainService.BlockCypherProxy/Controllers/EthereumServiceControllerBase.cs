...
{
    public abstract class EthereumServiceControllerBase : ControllerBase
    {
        protected IEthereumService _service;

        [HttpGet]
        public Task<EthereumBlockchain> BlockchainInfo()
        {
            return _service.GetBlockchainInfoAsync();
        }

        [HttpGet("balance")]
        public Task<EthereumAddressRecord> Balance(string address)
        {
        ...
        public Task<EthereumTXSkeleton> CreateTransaction([FromBody] EthereumTX transaction)
        {
            return _service.CreateTransactionAsync(transaction);
        }

        [HttpPost("sendtransaction")]
        public Task<EthereumTXSkeleton> BroadcastTransaction([FromBody] EthereumTXSkeleton transaction)
        {
